...
## Data Selection
Now we should select our relevant data, (in this case, we will  ID and Zip - we'll also convert Education into 3 dummy variables for each education level) and divide the set into training (60%) and validation (40%) sets.
```{r}
...
```
We see that the model has an accuracy of 95.4%
...
#2. We will now test the performance of our model across various k's on our validation set to find best k value.
```{r}
...
```
We see from the 'which' function that the best performing k in the range of 1 to 14 is `r which.max(accuracy.df$accuracy)`. This k is the most accurate for 3, and balances between overfitting and ignoring predictors.
...
```
From the confusion matrices the accuracies are Training: 97.92%; Validation: 95.33%; and Test: 95.8%. From this comparision we can understand that the accuracy of Training is more than Test and Validation. Hence  we have best training model
